...
public class BinarySearchTree<E extends Comparable<E>> {
    ...
    protected class RecBST {
        ...
        protected int computeHeight() {
            ...
            if (this.left != null) {
                leftHeight = 1 + this.left.computeHeight();
            }

            if (this.right != null) {
                rightHeight = 1 + this.right.computeHeight();
            }
        ...
        protected boolean insert(E label) {
            ///caso in cui l'elemento è già presente
            if(label.equals(this.label)){
                return false;
            }
            //se l'oggetto da inserire è minore dell'attuale
            if(this.label.compareTo(label) >= 0){
                //controllo che il figlio sinistro sia null
                if(this.left == null){
                    RecBST temp = new RecBST(label);
                    this.left = temp;
                    temp.parent = this;
                    return true;
                }
                //altrimenti chiamata ricorsiva al sotto albero sinistro
                return this.left.insert(label);
            }
            //se l'oggetto da inserire è maggiore dell'attuale
            else{
                if(this.right == null){
                    RecBST temp = new RecBST(label);
                    this.right = temp;
                ...
                //altrimenti chiamata ricorsiva al sotto albero destro
        ...
        protected RecBST search(E label) {
            ...
            if(label.equals(this.label))
                return this;
            //se label è minore di left, restituisce un negativo
        ...
        protected List<E> inOrderVisit() {
            ArrayList<E> list = new ArrayList<>(); //crea una lista di appoggio
            if(this.left != null){ //controlla che ci sia un figlio sinistro
                list.addAll(this.left.inOrderVisit()); //chiama ricorsivamente sul figlio sinistro e aggiunge tutto alla lista
            }
            list.add(this.label);
            if(this.right != null){ //controlla che ci sia un figlio destro
                list.addAll(this.right.inOrderVisit()); //chiama ricorsivamente sul figlio sinistro e aggiunge tutto alla lista
            }
            return list; //ritorna la lista ordinata
        }
        ...
        protected void deleteSelfLabel() {
            //caso in cui l'elemento da eliminare è una foglia
            if(this.left == null && this.right == null) {
                this.parent = null; //rimuove il collegamento con il padre
                this.label = null;
            }
            //ha solo un figlio sinistro
            else if(this.left != null && this.right == null){
                this.left.parent = this.parent; //il padre del figlio sinistro é il padre del nodo da eliminare
                //cancelliamo il nodo
                this.left = null;
                this.label = null;
                this.parent = null;
            }//ha solo un figlio destro
            else if(this.left == null && this.right != null){
                this.right.parent = this.parent; //il padre del figlio sinistro é il padre del nodo da eliminare
                //cancelliamo il nodo
                this.right = null;
                this.label = null;
                this.parent = null;
                ...
                RecBST successore = this.getSuccessorNode(); //contiene il successore del nodo da eliminare
                successore.deleteSelfLabel(); //eliminiamo il successore da dove si trova
                //posiziona correttamente il nodo successore
                successore.left = this.left;
                successore.right = this.right;
                successore.parent = this.parent;
                //cancelliamo il nodo
                this.right = null;
                this.left = null;
                this.label = null;
